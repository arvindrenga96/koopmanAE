...
def train(model, train_loader, lr, weight_decay, 
    ...
    # train_loader, val_loader = train_val_split(train_loader, val_split=0.1, random=False)
    ...
    for epoch in range(num_epochs):
        #print(epoch)
        len_of_trainloader = len(train_loader)

        for batch_idx, data_list in enumerate(train_loader):

            if batch_idx < len_of_trainloader - 1:
        ...
        #
            ...
            # schedule learning rate decay
                ...
                for k in range(steps):
                    if k == 0:
                        val_loss_fwd = criterion(out[k], data_list[k + 1].to(device))
                    else:
                        val_loss_fwd += criterion(out[k], data_list[k + 1].to(device))
        ...
        if (epoch) % 20 == 0:
                ...
                print("val loss: ", val_loss_fwd.item())
